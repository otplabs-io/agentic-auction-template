...
- **2026-09-27 completion session (cap 500 honored this time):** all 450 unique wines attempted; 440 lots valued -> 212 deals, 91 lots with no reliable price. Stage 2 (unrestricted retailer search) was run on every stage-1 miss; most stage-2 outcomes were "no vintage-specific price anywhere" for old/obscure vintages, recorded `insufficient`. WebFetch returned 403 on totalwine.com and shop.kermitlynch.com this run (besides wine.com / K&L); saratogawine.com, jjbuckley.com, yianniswine.com, whwc.com, leonandsonwine.com and craftcity.com worked.
- **Wine-Searcher "average retail" can sit well above the retailers you can actually see** (Alion 2021 avg $146 vs $108-$120 retailers; Auvigue Les Villages avg $56 vs $37-$56; Trimbach Schlossberg avg $135 vs $105). Where the average was not vintage-tied, or retailer listings were clearly lower, the lowest observed US retailer was used and labelled `ws_single_retailer`; where the vintage-specific average was used anyway, the source note names the retailer range so the discount sensitivity is visible.
- **Two-source rule for >65% discounts:** Grand Veneur CdP Vieilles Vignes 2022 (66.6%) shipped only after a fetched exact-vintage retailer page ($139.99 at Yiannis Wine, corroborated by Total Wine $149.99 and Wine-Searcher avg $146).
- **`test.js` hard-codes the region `Rhône`** but this week's export path reads `Rhône Valley`. Do not patch the HTML (that double-appends "Valley"); run a throwaway copy of `test.js` with `'Rhône'` -> `'Rhône Valley'` instead. Result: 90 passed / 4 failed (sample banner, six swatches, Rose accent, every-type filter) -- all four are the known data-shape failures.
