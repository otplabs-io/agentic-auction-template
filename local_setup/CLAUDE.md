...
Do not stop at the shortlist and do not wait for confirmation between steps. Screen → classify → value → build is the default. Pause only for a genuine anomaly (reserve above market, implausible range).
...
**2. Dessert / fortified** — drop by name/appellation: Sauternes, Barsac, SGN, late-harvest, ice wine/Eiswein, BA/TBA, Ausbruch, Vin Santo, passito/Recioto, Port, Madeira, Marsala, PX, Moscatel dessert styles, Rivesaltes/Banyuls/Maury.
...
## Publishing (optional, on request only)

Default deliverable is the local file. If asked to publish: copy to `output/index.html`, which the user uploads to `docs/index.html` via the GitHub web interface. Served at https://otplabs-io.github.io/agentic-auction-template/

The page carries `noindex, nofollow, noarchive` and `docs/robots.txt` disallows crawlers, because it's a ranked list of lots the user intends to bid on, published while the auction is open. Do not remove those. Pages caches ~10 minutes; check the masthead auction date before assuming a build failed.
