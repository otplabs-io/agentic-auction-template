...
**Localize queries to California.** Wine-Searcher's own URLs encode a location segment (e.g. `.../usa-ca-y` for California), and pricing/availability genuinely differs by locale on the live site. Since direct navigation is blocked, the closest available lever is appending the literal token `usa-ca-y` to the `WebSearch` query text (e.g. `site:wine-searcher.com <wine> <vintage> usa-ca-y`) — side-by-side testing on 2026-09-21 showed this consistently surfaces more specific, price-bearing results than the bare query (one wine went from zero usable results to a clean vintage-specific price with nothing else about the query changed). **Use the raw token `usa-ca-y`, not the word "California"** — spelling it out gets misread as a claim about the wine's origin ("this wine is from Italy, not California") rather than a location filter, and derails the answer.
