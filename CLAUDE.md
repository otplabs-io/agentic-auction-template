...
- **When the exact vintage isn't shown, fall back to the all-vintage average (`ws_allvintage`) before giving up.** Don't jump straight to `insufficient` just because a vintage-specific number isn't visible — try the all-vintage query for the same wine first and use that, noting the substitution. Reserve `insufficient` for wines with no price at *any* vintage grain. (Exception: don't paper over an actual cuvée/tier mismatch this way — see the rejection rules below. Falling back from "no 1998 price" to "this producer's all-vintage average" is fine; falling back from "no price for the base bottling" to "the Riserva's price" is not.)
...
- If no reliable value exists at all: source `"Insufficient data"`, `source_type` `insufficient`, exclude from deals, record under Unvalued. Very old vintages of drink-young wines, defunct micro-cuvées and unlisted small-production wines belong here rather than estimated from bad proxies.
- Unvalued lots still carry their Step 2 wine type.

**Wine-Searcher direct page fetches are bot-blocked** — confirmed 2026-09-21: the site presents a Cloudflare "press & hold to confirm you are human" challenge on direct navigation, and `WebFetch` gets a hard 403. Do not attempt to defeat this (no proxies, no headless workarounds) — it's explicit bot detection. This means the site's actual DOM/page structure (the per-vintage price table, the "Featured offer" merchant card, etc. — visible in a real signed-in browser) is not something this workflow can inspect directly; everything below works from `WebSearch` result titles and its synthesized answer text instead.

**Append the token `usa-ca-y` to queries — but understand what this is and isn't.** `WebSearch` has no location/region parameter at all (its schema is just `query`, `allowed_domains`, `blocked_domains`; the tool description says only "Web search is only available in the US," which isn't configurable). There is no way to actually run a search *from* California. What's being done instead is a text-matching trick: Wine-Searcher's own URLs encode a location segment (e.g. `.../usa-ca-y` for California), and appending that literal string to the query text (e.g. `site:wine-searcher.com <wine> <vintage> usa-ca-y`) changes which indexed pages/snippets the search surfaces — paired testing on 2026-09-21 took one wine from zero usable results to a clean vintage-specific price with nothing else about the query changed. This works because the token happens to match real indexed content, not because the request is geographically scoped. Don't describe this as "searching from California" when reporting or reasoning about it — it isn't. **Use the raw token `usa-ca-y`, not the word "California"** — spelling it out gets misread as a claim about the wine's origin ("this wine is from Italy, not California") rather than a location filter, and derails the answer.
